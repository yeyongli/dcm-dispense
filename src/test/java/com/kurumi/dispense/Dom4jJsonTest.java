	...
	public static void putSessionUrlListQueue_jsonStr(List<DicomInstanceMessage> list, String sessionId) {
		...
		if (list != null && list.size() > 0) {
			...
			for (DicomInstanceMessage dim : list) {
				...
				//帧的数量
				instanceUrl.setNumberFrame(dim.getNumberFrame());
		...
		if (list != null && list.size() > 0) {
			for (DicomInstanceMessage dim : list) { //size 152
				...
				try {
				 ...
				 map.put("sessionId", dim.getSessionId());
				 map.put("studyInstanceUID", dim.getStudyInstanceUID());
				 map.put("seriesInstanceUID", dim.getSeriesInstanceUID());
				 map.put("sOPInstanceUID", dim.getsOPInstanceUID());
				 map.put("instanceNumber", dim.getInstanceNumber());
				 map.put("characterSet", dim.getCharacterSet());
				 map.put("transferSyntaxUID", dim.getTransferSyntaxUID());
				 map.put("sOPClassUID", dim.getsOPClassUID());
				 map.put("modality", dim.getModality());
				 map.put("seriesNumber", dim.getSeriesNumber());
				 map.put("dcmFileStorageLocation", dim.getDcmFileStorageLocation());
				 map.put("studyID", dim.getStudyID());
				 map.put("studyDate", dim.getStudyDate());
				 map.put("studyTime", dim.getStudyTime());
				 map.put("accessionNumber", dim.getAccessionNumber());
				 map.put("numberFrame", numberFrame+"");
